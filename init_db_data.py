...
def gen_name():
    ...
    first_name = ['的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '这', '个', '上', '们', '来', '到', '时', '大', '地', '为',
    ...
    full_name = ''
    full_name = family_name[random.randint(100, 300) % len(family_name)]
    full_name += first_name[random.randint(1000, 3000) % len(first_name)]
    if random.randint(10, 30) % 2 == 0:
        full_name += first_name[random.randint(1000, 3000) % len(first_name)]

    return full_name


def gen_school_number():
    return random.randint(10 ** 3, 10 ** 4 - 1) + 2220000


def gen_phone():
    header = ['188', '134', '189', '180']
    return header[random.randint(10, 40) % len(header)] + str(random.randint(10 ** 7, 10 ** 8 - 1))
...
def load_covers():
    with open('douban-book-covers.json', 'r') as f:
        covers = f.readlines()
